...
class BlockManager:
    ...
    async def _vote(self, unconfirmed_block: Block):
        ...
        try:
            ...
            block_builder = BlockBuilder.from_new(unconfirmed_block, self.blockchain.tx_versioner)
            block_verifier = BlockVerifier.new(block_version, self.blockchain.tx_versioner)
            block_verifier.invoke_func = self.blockchain.score_invoke
            block_verifier.verify_invoke(block_builder, unconfirmed_block, self.blockchain.last_block)
        except Exception as e:
            exc = e
            logging.error(e)
            traceback.print_exc()
