...
class P2PRadiostationService(P2PService):
    def add_client(self, target: str) -> None:
        """ add p2p client for radiostation target (ip:port)
        TODO : change stub_manager to P2PClient
...
def get_radio_station_stub(radio_station_target):
    stub = None

    if conf.ENABLE_REP_RADIO_STATION:
        stub = StubManager.get_stub_manager_to_server(
            radio_station_target,
            loopchain_pb2_grpc.RadioStationStub,
            time_out_seconds=conf.CONNECTION_RETRY_TIMEOUT_TO_RS,
